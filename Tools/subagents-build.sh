#!/bin/zsh
# Builds the THIRD Bloom, the one a half-finished feature is tried out in, and
# installs it to ~/Applications/Bloom Subagents.app. It is Tools/dev-build.sh
...
#   bloomsub:             its own URL scheme, so the subagents copy cannot steal a
...
#   the name      "Bloom Subagents" in the Dock, in the Cmd-Tab switcher and as the
...
bloom_refuse_if_own_host "$DEST" "$BLOOM_SUB_DB"
...
plist "Set :CFBundleName Bloom Subagents"
plist "Set :CFBundleDisplayName Bloom Subagents"
plist "Set :CFBundleIdentifier $BLOOM_SUB_BUNDLE_ID"
plist "Set :CFBundleURLTypes:0:CFBundleURLName $BLOOM_SUB_BUNDLE_ID.deeplink"
plist "Set :CFBundleURLTypes:0:CFBundleURLSchemes:0 bloomsub"
plist "Set :NSServices:0:NSPortName Bloom Subagents"
plist "Set :NSServices:0:NSMenuItem:default New Bloom Subagents Workspace"
...
# from the Dock. A build launched as ~/Applications/Bloom Subagents.app/Contents/MacOS/Bloom
...
# "Bloom Subagents" directory rather than to the real one. So this key is belt now, not
...
plist "Add :LSEnvironment:BLOOM_DB_PATH string $BLOOM_SUB_DB"
...
python3 Tools/icon/dev-tint.py "$WORK/Resources/Bloom.icon/icon.json" 0.25

# The window title, in all three of the places that set one.
#
# Three, and finding the third took two subagents builds that came out titled "Bloom"
...
# fewer inline in the code rather than in a string table, and "[SUB] Bloom" is
...
mkdir -p /tmp/bloom-sub-build
ln -sfn /tmp/bloom-sub-build "$WORK/.build"
...
BUILD_LOG=/tmp/bloom-sub-build.log
...
if [ "$LAUNCH" -eq 1 ]; then
  ...
  if ps -E -p "$running" 2>/dev/null | grep -q "BLOOM_DB_PATH=$BLOOM_SUB_DB"; then
    ...
    # from the dev bundle id and lands on Bloom Subagents anyway. But a missing key means
