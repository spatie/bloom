...
def configuration(args):
    digest = getattr(args, 'maintenance_key_sha256', None)
    if digest is not None and re.fullmatch(r'[0-9a-f]{64}', digest) is None:
        fail('invalid_maintenance_key', 'The maintenance credential digest is invalid.',
             'Use the maintenance access key generated by Bloom. The raw key must not be uploaded.')
...
def maintenance_installation(args):
    value = MaintenanceInstallation(args, protected_system_path)
    if not value.enabled:
        return None
    args.runtime_socket = socket_path(args)
    return value


def maintenance_call(operation):
    try:
        return operation()
    except MaintenanceInstallFailure as error:
        fail(error.code, error.message, error.recovery)


def maintenance_source():
    if 'MAINTENANCE_SUPERVISOR_SOURCE' in globals():
        return MAINTENANCE_SUPERVISOR_SOURCE
    return pathlib.Path(__file__).with_name('bloom-maintenance.py').read_text()


def maintenance_docker_source():
    if 'MAINTENANCE_DOCKER_SOURCE' in globals():
        return MAINTENANCE_DOCKER_SOURCE
...
def managed_service_state(args):
    ...
    if (not set(fields).issubset(values) or values["LoadState"] != "loaded" or values["FragmentPath"] != str(unit)
            or values["User"] != ("root" if supervised else args.user) or values["Group"] != ("root" if supervised else args.user) or values["DropInPaths"]
...
def verify_managed_process(args, account, state, proc=pathlib.Path("/proc")):
    try:
        ...
        root = proc / str(pid)
        supervised = maintenance_installation(args)
        expected = "/usr/bin/python3" if supervised else str(args.install_root / "current/bin/bloom-server")
        expected_arguments = ([os.fsencode(expected), os.fsencode(supervised.launcher), b"--config", os.fsencode(supervised.config_path), b""]
                              if supervised else [os.fsencode(expected), b"serve", b"--data-dir", os.fsencode(args.data_dir), b""])
        ...
        if [int(value) for value in fields["Uid"].split()] != [0 if supervised else account.pw_uid] * 4:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if (result is not None and result.returncode == 0 and stat.S_ISSOCK(info.st_mode)
                    ...
                    and (supervised is None or supervised.socket_ready(pwd.getpwnam(args.user)))):
...
def install(args):
    ...
    with tempfile.TemporaryDirectory(prefix="bloom-server-install-", dir="/var/tmp") as temporary:
        ...
        if supervised is not None:
            maintenance_call(supervised.validate_digest)
            maintenance_call(lambda: supervised.validate_bundle(bundle))
        ...
        try:
            ...
            if supervised is not None:
                maintenance_call(supervised.rollback)
