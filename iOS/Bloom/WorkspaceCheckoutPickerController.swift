...
final class WorkspaceCheckoutPickerController: UITableViewController, UISearchResultsUpdating {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Open reference", primaryAction: UIAction { [weak self] _ in self?.reference() })
    ...
    private func load() {
        work = Task { [weak self, service, project] in
            do {
                ...
                self.options = result; self.tableView.reloadData()
            } catch { if !Task.isCancelled { self?.show(error) } }
        }
    }
    override func numberOfSections(in tableView: UITableView) -> Int { 3 }
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        section == 0 ? 1 : (section == 1 ? requests.count : branches.count)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 { content.text = "New branch"; content.image = UIImage(systemName: "plus") } else if indexPath.section == 1 {
    ...
    private func reference() {
        ...
        alert.addTextField { $0.placeholder = "#123 or GitHub URL"; $0.autocapitalizationType = .none }
