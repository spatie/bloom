...
func TestRPCRequiresAuthenticationBeforeConnectingAndPreservesIDs(t *testing.T) {
	...
	body := `{"version":16,"id":"00000000-0000-0000-0000-000000000001","operation":{"hello":{}}}`
...
func TestRPCRejectsMalformedAndOversizedBodies(t *testing.T) {
	...
	for _, body := range []string{
		...
		`{"version":16,"id":"00000000-0000-0000-0000-000000000001","operation":{"hello":{},"catalogue":{}}}`,
		`{"version":16,"id":"00000000-0000-0000-0000-000000000001","operation":{"hello":{}}}{"extra":true}`,
	} {
...
func TestRPCKeepsNegotiatedVersionsAndLimitsHelloDowngrades(t *testing.T) {
	for _, test := range []struct {
		...
		{"helloNegotiation", "hello", `{"failure":{"_0":"Incompatible Bloom server protocol. Update the client and server."}}`, 16, 13, 200},
		{"noMutationDowngrade", "send", `{"failure":{"_0":"Incompatible Bloom server protocol. Update the client and server."}}`, 16, 13, 502},
		{"unknownVersion", "hello", `{"failure":{"_0":"Incompatible Bloom server protocol. Update the client and server."}}`, 16, 17, 502},
		{"wrongHelloVersion", "hello", `{"hello":{"name":"test"}}`, 16, 13, 502},
	} {
...
func TestTerminalStreamRequiresControlAccessAndRelaysBinaryInput(t *testing.T) {
	...
	count := runtimeFixture(t, f, func(connection net.Conn, request rpcRequest) {
		...
		json.NewEncoder(connection).Encode(map[string]any{"version": protocolVersion, "id": request.ID, "result": map[string]any{"text": map[string]string{"_0": path}}})
	})
