...
func (server *Server) terminal(writer http.ResponseWriter, request *http.Request, config Config) {
	...
	id := requestID()
	body, _ := json.Marshal(map[string]any{"version": 11, "id": id, "operation": map[string]any{"terminalStream": map[string]string{"workspaceID": query.Get("workspace_id"), "name": query.Get("name")}}})
