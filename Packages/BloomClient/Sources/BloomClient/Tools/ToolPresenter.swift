...
public enum ToolPresenter {
    ...
    private static func crewRow(
        ...
        let detail = oneLine(name, limit: ToolHostIdentity.crewNameLimit)
        return ToolPresentation(
            glyph: glyph,
            label: label,
            detail: detail,
    ...
    private static func mcp(name: String, input: JSONValue) -> ToolPresentation {
        ...
        // convention: `ToolHostIdentity.bridgeServerName` records the measurement behind it, that a
        ...
        let isBloom = server == ToolHostIdentity.bridgeServerName
