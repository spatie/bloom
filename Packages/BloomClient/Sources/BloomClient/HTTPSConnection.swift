...
public final class HTTPSConnection: Sendable {
    ...
    public init(baseURL: URL, accessToken: @escaping AccessToken, session: URLSession) throws {
        self.baseURL = try Self.origin(baseURL.absoluteString)
        self.accessToken = accessToken
    ...
    public static func origin(_ text: String) throws -> URL {
        guard var components = URLComponents(string: text), components.scheme?.lowercased() == "https",
            ...
            throw ConnectionFailure("Enter the server's HTTPS address without a path or credentials.")
        }
        components.scheme = "https"
        components.host = host.lowercased()
        components.path = ""
        guard let url = components.url else { throw ConnectionFailure("Enter a valid HTTPS server address.") }
        return url
    }

    public func exchange(_ body: Data, timeout: Duration = .seconds(660)) async throws -> Data {
        let token = try await accessToken()
        guard !token.isEmpty, !token.contains("\r"), !token.contains("\n") else { throw ConnectionFailure("Sign in to this server first.") }
        ...
        guard let response = response as? HTTPURLResponse else { throw ConnectionFailure("The server returned an invalid HTTPS response.") }
        if response.statusCode == 401 || response.statusCode == 403 {
            throw ConnectionRefusal("Server access expired or was revoked. Sign in again.")
        }
        guard response.statusCode == 200, data.count <= 16_777_216 else {
            throw ConnectionFailure("The HTTPS gateway could not complete this request (HTTP \(response.statusCode)). Refresh before retrying with the same request ID.")
        }
        return data
    }
    ...
    public func terminal(workspaceID: String, name: String) async throws -> URLSessionWebSocketTask {
        var components = URLComponents(url: baseURL.appendingPathComponent("v1/terminal"), resolvingAgainstBaseURL: false)
        components?.scheme = "wss"
        components?.queryItems = [URLQueryItem(name: "workspace_id", value: workspaceID), URLQueryItem(name: "name", value: name)]
        guard let url = components?.url else { throw ConnectionFailure("Invalid terminal address.") }
        let token = try await accessToken()
        guard !token.isEmpty, !token.contains("\r"), !token.contains("\n") else { throw ConnectionFailure("Sign in to this server first.") }
