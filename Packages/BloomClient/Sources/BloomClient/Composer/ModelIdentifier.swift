...
public extension ModelIdentifier {
    static func resolve(_ raw: String, codexModels: [CodexModel]) -> ModelIdentifier {
        resolve(raw, models: [.codex: codexModels.map(\.agentModel)])
    }

    static func correction(model raw: String, on kind: AgentKind, hasSpoken: Bool,
                           codexModels: [CodexModel]) -> ModelIdentifier? {
        correction(model: raw, on: kind, hasSpoken: hasSpoken, models: [.codex: codexModels.map(\.agentModel)])
    }
}
