...
class ClientTests(unittest.TestCase):
    def test_negotiates_only_known_older_hello(self):
        ...
        self.assertEqual([16, 12, 12], [c["version"] for c in transport.commands])
    ...
    def test_refuses_unknown_version(self):
        with self.assertRaises(BloomError):
            Client(ScriptedTransport([(17, {"hello": {"name": "server"}}, False)])).connect()

    def test_no_automatic_command_retry(self):
        transport = ScriptedTransport([(16, {"hello": {"name": "server"}}, False), (16, {"failure": {"_0": "File changed"}}, False)])
        client = Client(transport)
        client.connect()
