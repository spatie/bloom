...
workspace_actions = {
    ...
    **{name: obj() for name in ["runSetup", "setupOutput", "archivePreview", "restore", "deletePreview", "files", "pullRequest", "runScripts", "browserAddress", "push", "notes"]},
    "archive": obj({"confirmation": U, "removingDocker": optional(B)}, ["confirmation"]), "delete": obj({"confirmation": U}), "runScript": obj({"id": S}), "download": obj({"path": S}),
...
project_actions = {
    ...
    "removalPreview": obj(), "remove": obj({"confirmation": U}),
}
creation_actions = {
...
operations = {
    **{name: obj() for name in ["hello", "diagnostics", "storage", "catalogue"]},
    ...
    "removeStorageLeftovers": obj({"workspaceIDs": array(S)}),
    ...
    "skills": payload(ref("ServerSkillsRequest")),
...
results = {
    ...
    **{name: payload(S) for name in ["patch", "text"]}, "changes": payload(array(ref("ChangedFile"))),
    ...
    "removalPreview": payload(ref("RemovalPreview")),
    ...
    "storageLeftoverRemoval": payload(ref("ServerStorageLeftoverRemoval")),
    "maintenance": payload(ref("ServerMaintenanceResponse")),
    "skills": payload(ref("ServerSkillsResponse")),
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "$id": f"https://runbloom.app/protocol/bloom-v{VERSION}.schema.json",
            ...
            "$defs": {
                **json.loads((ROOT / "Protocol/maintenance-v1.schema.json").read_text())["$defs"],
                "ServerSkillAgent": {"enum": ["claude", "codex"]},
                "ServerSkillSource": {"enum": ["personal", "git", "project", "unmanaged"]},
                "ServerSkillFile": obj({"path": S, "data": D, "isExecutable": optional(B)}, ["path", "data"]),
                "ServerSkillsRequest": obj({
                    "action": {"enum": ["inspect", "read", "previewImport", "previewGit", "apply", "setEnabled", "remove"]},
                    "workspaceID": optional(S), "files": optional(array(ref("ServerSkillFile"))),
                    "repositoryURL": optional(S), "ref": optional(S), "collectionID": optional(S),
                    "planID": optional(S), "skillID": optional(S), "revision": optional(S),
                    "selectedSkillNames": optional(array(S)), "agents": optional(array(ref("ServerSkillAgent")))}, ["action"]),
                "ServerSkill": obj({"id": S, "name": S, "description": S, "source": ref("ServerSkillSource"),
                    "revision": S, "enabledAgents": array(ref("ServerSkillAgent")), "fileCount": I, "byteCount": I,
                    "repositoryURL": optional(S), "commit": optional(S), "collectionID": optional(S),
                    "path": S, "warning": optional(S), "filePaths": array(S)},
                    ["id", "name", "description", "source", "revision", "enabledAgents", "fileCount", "byteCount", "path", "filePaths"]),
                "ServerSkillsPlan": obj({"id": S, "source": ref("ServerSkillSource"), "skills": array(ref("ServerSkill")),
                    "repositoryURL": optional(S), "commit": optional(S), "collectionID": S,
                    "expiresAt": {"type": "number"}, "warnings": array(S)},
                    ["id", "source", "skills", "collectionID", "expiresAt", "warnings"]),
                "ServerSkillsResponse": obj({"skills": array(ref("ServerSkill")), "plan": optional(ref("ServerSkillsPlan")),
                    "content": optional(S), "warnings": array(S)}, ["skills", "warnings"]),
                ...
                "ArchivePreview": obj({"id": U, "workspace": ref("Workspace"), "report": ref("WorkspaceSafetyReport"),
                                       ...
                                       "docker": optional(ref("ArchiveDockerFootprint"))},
                                      ["id", "workspace", "report", "hazards", "createdAt"]),
                "WorkspaceDockerResource": obj({"kind": {"enum": ["container", "volume", "network"]}, "name": S,
                                                "composeProject": optional(S), "isRunning": B, "sizeLabel": optional(S)},
                                               ["kind", "name", "isRunning"], True),
                "ArchiveDockerFootprint": obj({"resources": array(ref("WorkspaceDockerResource"))}, extra=True),
                "RemovalPreview": obj({"id": U, "title": S, "message": S, "confirmLabel": S, "cancelLabel": S,
                                       "blocker": optional(S), "createdAt": {"type": "number"}},
                                      ["id", "title", "message", "confirmLabel", "cancelLabel", "createdAt"]),
                ...
                "ComposerState": obj({"controls": ref("ComposerControls"), "models": array(R), "commands": array(R), "styles": array(R),
                                      "availableAgents": optional(array(S)), "authentication": optional(array(ref("AgentAuthentication"))),
                                      "codexSpeeds": optional({"type": "object", "additionalProperties": ref("CodexSpeed")})}, ["controls", "models", "commands", "styles"], True),
                "CodexSpeed": obj({"isFast": B, "supportsFast": B}),
                ...
                "ServerStorageReport": obj({"checkedAt": {"type": "number"}, "totalBytes": optional(I), "freeBytes": optional(I),
                                            ...
                                            "usage": array(ref("ServerStorageUsage")), "notes": array(S),
                                            "leftovers": optional(array(ref("ServerStorageLeftover"))), "leftoversMessage": optional(S)},
                                           ["checkedAt", "dockerState", "usage", "notes"], True),
                "ServerStorageLeftover": obj({"workspaceID": S, "workspaceName": optional(S), "owner": {"enum": ["archived", "unknown"]},
                                              "resources": array(ref("WorkspaceDockerResource"))},
                                             ["workspaceID", "owner", "resources"], True),
                "ServerStorageLeftoverOutcome": obj({"workspaceID": S, "status": {"enum": ["completed", "uncertain", "failed"]}, "message": S},
                                                   extra=True),
                "ServerStorageLeftoverRemoval": obj({"outcomes": array(ref("ServerStorageLeftoverOutcome")),
                                                     "report": optional(ref("ServerStorageReport"))}, ["outcomes"], True),
                ...
                "ServerDiagnostics": obj({"checkedAt": {"type": "number"}, "hostname": S, "operatingSystem": S,
                                          ...
                                          "authentication": optional(array(ref("AgentAuthentication"))), "storageManagement": optional(B), "maintenanceManagement": optional(B), "skillManagement": optional(B)},
