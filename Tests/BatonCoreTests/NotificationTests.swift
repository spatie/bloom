...
/// Baton runs several agents at once, so a notification is only useful when it names a workspace
...
/// live in BatonCore precisely so they can be asserted on without a bundle or a permission.
...
struct NotificationPreferencesTests {
    ...
    private func makeDefaults() throws -> (UserDefaults, String) {
        let name = "baton.notifications.\(UUID().uuidString)"
...
struct NotificationDigestTests {
    ...
    func singleKeepsTheWorkspaceName() {
        ...
        #expect(prepared?.identifier == "baton.turnFinished.w1")
    }
    ...
    func collapsesABatchIntoASummary() {
        ...
        #expect(prepared?.workspaceID == "w1")
        #expect(prepared?.threadIdentifier == "baton.turnFinished")
    }
    ...
    func everyEventHasASummary() {
        for event in NotificationEvent.allCases {
            ...
            #expect(prepared?.identifier == "baton.\(event.rawValue).digest")
        }
    }
