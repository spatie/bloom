...
/// `WorkspaceManager.archive` is the only thing in Baton that deletes a git worktree and can
...
struct WorkspaceArchiveTests {
    ...
    func ignoredFilesAreInvisibleToTheReport() async throws {
        ...
        // Baton itself copies `.env*` into every new worktree, so this is the file most likely to
        ...
        // deletes them silently. Baton copies `.env*` into every worktree, so this was the
