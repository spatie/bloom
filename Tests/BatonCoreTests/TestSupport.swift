...
enum TestScratch {
    ...
    static func path(_ name: String) -> String {
        let root = directory ?? {
            let fallback = NSTemporaryDirectory() + "baton-unscoped-\(UUID().uuidString)"
...
struct ScratchDirectoryTrait: TestTrait, SuiteTrait, TestScoping {
    ...
    func provideScope(
        ...
        let root = NSTemporaryDirectory() + "baton-scratch-\(UUID().uuidString)"
...
struct TempRepo {
    ...
    init(defaultBranch: String = "main") async throws {
        path = TestScratch.unique("baton-git")
    ...
    /// `~/baton/workspaces` on the machine where it was found. Reading the worktree list here
