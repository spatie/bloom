...
struct GhosttyConfigTests {
    ...
    func palettePerSlot() {
        ...
        // Untouched slots keep Ghostty's own defaults rather than borrowing Baton's palette.
    ...
    @Test("no config file anywhere means no opinion, so Baton keeps its own colours")
