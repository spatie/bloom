...
struct TabStripVisibilityTests {
    @Test("one tab hides the strip, however many panes it is split into")
    func singleTab() {
        #expect(!TabStripVisibility.isShown(tabCount: 1))
    }

    @Test("a second tab brings the strip back")
    func secondTab() {
        #expect(TabStripVisibility.isShown(tabCount: 2))
        #expect(TabStripVisibility.isShown(tabCount: 5))
    }

    @Test("a workspace with no tab has no strip")
    func noTab() {
        #expect(!TabStripVisibility.isShown(tabCount: 0))
        #expect(!TabStripVisibility.isShown(tabCount: 0, isRenaming: true))
    }

    @Test("renaming a lone tab shows the strip for as long as the field is open")
    func renaming() {
        #expect(TabStripVisibility.isShown(tabCount: 1, isRenaming: true))
    }
}
