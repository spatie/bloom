...
protocol WorkspaceFileReview: WorkspaceFileListing {
    ...
    /// The same patch, with git's reason for refusing instead of an empty string. See
    /// `DiffView.load`, which draws that reason rather than "No diff".
    ...
    /// What the comment anchors and the between-hunks expanders resolve against, which for a
    /// scope in the past is that commit's copy rather than the one in the worktree.
    func reviewContents(of file: ChangedFile) async -> String?
