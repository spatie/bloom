...
/// History and files share the inspector; the existing centre review keeps the room a diff
/// needs. A resizable divider lets a long history grow without squeezing code into the sidebar.
    ...
    var body: some View {
        VSplitView {
            ChangesHistoryList(model: model)
                .frame(minHeight: 140, idealHeight: 250, maxHeight: 420)
            Group {
...
struct ChangesHistoryList: View {
    ...
    private var scopes: [DiffScope] { [.all, .uncommitted] + model.branchCommits.commits.map(DiffScope.commit) }
