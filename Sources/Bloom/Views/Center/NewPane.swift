...
/// the pane's two split submenus now offer the same three items, and three literals in two files
/// drift the first time one of them is renamed. A menu where Browser wears a globe in one place
/// and something else in another is worse than a menu with no glyphs at all.
///
