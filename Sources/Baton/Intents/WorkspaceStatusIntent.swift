...
struct WorkspaceStatusIntent: AppIntent {
    ...
    static let description = IntentDescription(
        """
        Returns one Baton workspace with its status, whether an agent is running, its diff stat \
    ...
    @Parameter(
        title: "Ask GitHub",
        description: "Looks the pull request up with gh. Turn it off to answer from Baton's database alone.",
        default: true
    )
