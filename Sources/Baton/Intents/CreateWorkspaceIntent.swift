...
/// The one people will actually automate: hand Baton a project and a sentence, get a workspace.
struct CreateWorkspaceIntent: AppIntent {
    static let title: LocalizedStringResource = "Create Workspace"

    static let description = IntentDescription(
        """
        Cuts a git worktree in a Baton project, opens it, and starts an agent on the prompt. \
        Returns the workspace once Baton has created it.
        """,
        categoryName: "Workspaces",
        resultValueName: "Workspace"
    )

    /// Creating a workspace starts a `claude` process that Baton has to own for the whole of its
    ...
    @Parameter(title: "Project", description: "The Baton project to cut the worktree from.")
    var project: ProjectEntity

    @Parameter(
        title: "Prompt",
        description: "What the agent should do. Baton names the workspace and its branch from this.",
    ...
    /// `baton://` is the link Baton already accepts from scripts and from Conductor, so an intent
    ...
    private func link(repo: Repo) -> URL {
        ...
        return URL(string: "baton://?prompt=\(escape(prompt))&path=\(escape(repo.path))")!
    }
