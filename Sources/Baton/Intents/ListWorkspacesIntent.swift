...
struct ListWorkspacesIntent: AppIntent {
    ...
    static let description = IntentDescription(
        """
        Returns Baton's workspaces with their status and diff size. Reads Baton's database \
        directly, so it answers whether or not Baton is open.
        """,
    ...
    private func dialog(for entities: [WorkspaceEntity]) -> IntentDialog {
        ...
        if entities.isEmpty { return "Baton has no workspaces." }
