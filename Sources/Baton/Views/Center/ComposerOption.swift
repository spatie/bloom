...
struct ComposerOption: Identifiable, Hashable {
    ...
    /// Falls back to a tidied form of the raw value, so a model set in a settings file that Baton
    ...
    ///
    /// The vendor prefix goes: inside Baton every model is a Claude model, so the word carries no
