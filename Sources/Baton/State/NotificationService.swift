...
/// in `NotificationPolicy` and `NotificationDigest`, in BatonCore, where it can be tested without
...
final class NotificationService {
    ...
    /// permission in System Settings while Baton is running, and a toggle that keeps claiming to
    ...
    /// Handed the app state once it exists, the same way `BatonAppDelegate` is. The suppression
    ...
    /// Whether macOS is currently throwing away everything Baton sends. Drives the warning in the
    /// settings pane.
    ...
    func openSystemSettings() {
        // The Ventura-and-later pane id. The `id` query selects Baton's own row, so the switch the
    ...
    func sendTestNotification() {
        deliver(PreparedNotification(
            identifier: "baton.test",
            threadIdentifier: "baton.test",
            title: "Baton",
    ...
    /// Watches CI on the pull requests Baton already knows about.
    ///
