...
/// alive across a quit.
///
/// One session per Baton *pane*, never per window: `SplitLayout` already owns the geometry, so
...
public enum TmuxSessions {
    ...
    public static let sessionPrefix = "baton"
    ...
    public static func isBatonSession(_ name: String) -> Bool {
        parts(of: name) != nil
    }
    ...
    /// is not loaded into shells Baton drew, so their prefix key and status line stay theirs.
    ///
    /// The socket is per database, so an instance pointed at `BATON_DB_PATH` cannot adopt or sweep
    /// the sessions of the instance holding the user's real workspaces.
    public static func socketName(databasePath: String) -> String {
        "baton-" + fingerprint(databasePath)
    }
    ...
    /// Sessions that nothing in Baton can ever reach again.
    ///
    ...
    /// edit outside Baton left behind.
    ...
    /// killed. Nothing running is allowed to depend on Baton having remembered it.
    ...
    public static func configuration(defaultShell: String) -> String {
        """
        # Written by Baton on every launch. Edits will be overwritten.
        #
        # This file configures a private tmux server that holds one shell per Baton terminal pane.
...
/// One resolved tmux binary on one private socket, and every command line Baton sends it.
///
...
public struct TmuxCommand: Sendable, Equatable {
    ...
    /// Re-applies the configuration to a server that was already running when Baton launched,
